...
- [x] **Step 4:** PROBE RESULTS (2026-07-07):
  - NSWorkspace activation notifications deliver ONLY on a main-thread runloop (0/3 on background thread, 3/3 on main). → `window_activity` uses a pure-Python **sidecar subprocess** (`window_sidecar.py`, run with `paths.venv_python()`): its main thread pumps the runloop + does enrichment, prints one JSON line per event; the in-kernel `run()` reads stdout lines via asyncio subprocess pipe.
  - FSEvents works fully on a background thread (file-level events, `FSEventStreamSetExclusionPaths` honored, UseCFTypes gives str paths). In-kernel as planned.
  - Process tap: `CATapDescription`/`AudioHardwareCreateProcessTap`/`AudioHardwareCreateAggregateDevice` work via pyobjc — composition keys are `name`/`uid`/`private`/`taps` (list of `{"uid": <tap UUID string>, "drift": True}`)/`tapautostart`. BUT tap audio is only visible inside a HAL IOProc, and pyobjc cannot round-trip `AudioDeviceIOProcID` — the IO leg must use **ctypes** (`AudioDeviceCreateIOProcID` with `CFUNCTYPE` callback, `AudioDeviceStart/Stop/DestroyIOProcID`). Tap format on this machine: lpcm float32 interleaved, 48kHz, 2ch (query via `AudioObjectGetPropertyData` `sfmt`/`inpt`). Notetaker records tap → `system.raw` (float32) + mic via sounddevice → `mic.raw` (int16), merges with `ffmpeg amix` at finalize.
