...
struct ActivityWindow: View {
    ...
    private func row(_ line: ActivityLine) -> some View {
        Group {
            ...
            case .nudgeDone:
                Text("  ✓ done").foregroundStyle(.green).opacity(0.8)
            case .paiCommand:
                Text(line.raw).foregroundStyle(.cyan)
            case .paiSay:
                Text(line.raw).foregroundStyle(.purple)
            case .commandOutput:
            ...
            case .other:
                Text(line.raw).foregroundStyle(.tertiary)
            }
        }
        .font(logFont)
    }
