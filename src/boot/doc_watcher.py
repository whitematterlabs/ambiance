...
Watches $PAI_ROOT/usr/share/doc/ (recursive) for any file event. On each
one, emits a `doc-watcher:review-doc` event carrying the doc's path relative
to PAI_ROOT. The kernel routes it via wake_on to the librarian, which reads
...
async def run() -> None:
    ...
    observer.start()
    print(f"[doc-watcher] started on {DOC_DIR}", flush=True)
