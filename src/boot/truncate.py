...
full text to `PAI_ROOT/tmp/` and cites the PAI-addressable `/tmp/...` path in
the footer so the PAI can `tail`/`grep` the rest itself.
"""
...
def spill(text: str, *, prefix: str) -> str:
    """Write `text` to `PAI_ROOT/tmp/<prefix>-<8hex>.log`; return the
    PAI-view path (`/tmp/<name>`), which the shell tools' FHS rewrite maps
    back to the host file. The clean boot phase wipes PAI_ROOT/tmp."""
    name = f"{prefix}-{secrets.token_hex(4)}.log"
    host_dir = paths.PAI_ROOT / "tmp"
    host_dir.mkdir(parents=True, exist_ok=True)
    (host_dir / name).write_text(text, encoding="utf-8")
    return f"/tmp/{name}"


def cap_tail_for_model(rendered: str, *, slug: str, tool: str) -> str:
    """Model-bound copy of a bash/shell result: last 2000 lines / 50KB.

    When truncated, the full text is spilled to a PAI-addressable temp file
    cited in the footer, so the PAI can read the rest with `tail`/`grep`.
    Untruncated text passes through with no file written."""
    t = truncate_tail(rendered)
    if not t.truncated:
        return rendered
    safe_slug = re.sub(r"[^A-Za-z0-9_-]+", "-", slug or "pai")
    path = spill(rendered, prefix=f"{tool}-{safe_slug}")
