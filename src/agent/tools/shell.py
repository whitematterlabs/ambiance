...
Backed by a long-lived bash subprocess running under a PTY so:
  - state (cwd, env, history) persists across tool calls,
  - interactive CLIs and TUIs (vim, claude, npm init) get a real PTY,
  - the owner can `tmux -S /tmp/pai-tmux-<user>.sock attach -t pai-<user>`
...
Two modes via the same tool:
  - `command`: run a bash command to completion. The command is
    ...
    exit code arrives on a third pipe. The writer-side parser
...
from ._shell_common import ShellResult
...
TOOL_DESCRIPTION = (
    ...
    "slow CLIs. The cap is the cap; don't fight it, background "
    "instead.\n\n"
    ...
    "tool calls for this agent's lifetime, so PIDs and `jobs` survive "
    "between invocations."
)
...
def _session_slug() -> str:
    """One agent process = one member = one session; named after the uid."""
    try:
...
def _socket_path(slug: str) -> Path:
    return Path(tempfile.gettempdir()) / f"pai-tmux-{slug}.sock"


def _live_log_path(slug: str) -> Path:
    base = os.environ.get("XDG_STATE_HOME")
    p = (Path(base) if base else Path.home() / ".local" / "state") / "pai"
    p.mkdir(parents=True, exist_ok=True)
    return p / f"shell-{slug}.live"
...
def _setup_slave_termios(fd: int) -> None:
    ...
    # line. With ICANON on, the tty caps a canonical input line at MAX_CANON
    # (1024 bytes on some platforms, 4096 on Linux) and silently drops the
    # rest — long commands (heredocs, large printfs) get truncated, the
    # `_pai_run` payload arrives malformed, and the eval fails silently.
    # Leave ISIG on so a `\x03` written to the master still raises SIGINT in
    # the foreground pgroup (the timeout / interrupt paths depend on this).
...
async def _write_all_to_master(fd: int, data: bytes) -> None:
    ...
    # the slave's input buffer fills (~4KB). Loop until everything lands,
    # yielding to the event loop on EAGAIN so pipe readers drain.
    n = 0
    while n < len(data):
        try:
...
def _spawn_viewer_tmux(slug: str, log_path: Path) -> None:
    ...
    watch decoded commands + outputs in real time. Tmux is not in the
    parsing path; this is display-only.
    """
    ...
    try:
        ...
        # Viewer is best-effort. The session keeps working without it.
        pass
...
async def shutdown_all() -> None:
    """Tear down every live bash subprocess. Called on agent shutdown."""
    for sess in list(_sessions.values()):
        _force_teardown(sess)


def interrupt(slug: Optional[str] = None) -> bool:
    """Send Ctrl-C to the foreground process group of the session's PTY.

    Used by the turn-cancel path to interrupt a long-running command
    without killing the persistent shell. The init script runs `set -m`
    so foreground commands sit in their own pgroup and bash itself
    doesn't share their SIGINT.
    """
    sess = _sessions.get(slug or _session_slug())
    if sess is None:
        return False
    try:
...
async def run(
    ...
    if command and keys:
        return ShellResult(
            stdout="",
            stderr="shell tool: pass `command` OR `keys`, not both",
            exit_code=-1,
        )
    if not command and not keys:
        return ShellResult(
            stdout="",
            stderr="shell tool: must pass `command` or `keys`",
            exit_code=-1,
        )
    ...
    if env:
        proc_env.update(env)
    cwd = Path(proc_env.get("HOME") or Path.home())
