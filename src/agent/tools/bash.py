...
TOOL_DESCRIPTION = (
    ...
    "Defaults: cwd is your home directory; pass `cwd` to override (must "
    "exist). `timeout_ms` defaults to 120000 (2 min), max 600000 (10 min). "
    "On timeout the process is sent SIGTERM, then SIGKILL after a brief "
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "cwd": {
                "type": "string",
                "description": (
                    "Optional working directory. Defaults to your home "
                    "directory. Must exist; otherwise the call returns "
                    "exit_code -1."
                ),
            },
            "timeout_ms": {
...
def _build_env(extra: Optional[dict]) -> dict:
    """Build a clean env: process env + TERM=dumb.
...
async def run(
    ...
    try:
        ...
        # Owner interrupt: the turn task is being cancelled with this
